...
## Gemini coach summary deployment (secure API key handoff)

This project is already wired so the browser **does not** need the Gemini API key.

- Frontend calls: `POST /api/gemini/coach-summary`
- Backend calls Gemini using server env var: `GEMINI_API_KEY`

### 1) Backend environment variables (set on server)

Set these on the backend runtime (not in frontend build vars):
...
```env
# Dev (using Vite proxy): leave empty
VITE_API_BASE_URL=

# Prod (if frontend and backend are on different origins)
# VITE_API_BASE_URL=https://your-backend-domain.com

VITE_GEMINI_ENABLED=true
```

### 3) Quick readiness check after backend injects key

Once backend is running, check:

```sh
curl http://127.0.0.1:8000/api/gemini/status
```
...
If `ready` is `true`, backend can serve Gemini coach requests immediately.
